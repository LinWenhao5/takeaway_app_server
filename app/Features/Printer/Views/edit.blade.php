...
@section('title', __('printer.title_edit'))

@section('breadcrumb')
<nav aria-label="breadcrumb">
    <ol class="breadcrumb mb-3">
        <li class="breadcrumb-item"><a href="{{ route('admin.printers.index') }}">@lang('printer.title_index')</a></li>
        <li class="breadcrumb-item active" aria-current="page">@lang('printer.edit')</li>
    </ol>
</nav>
@endsection
...
<div class="container" style="max-width: 800px;">
    <div class="mb-4">
        <h1 class="mb-0">@lang('printer.title_edit')</h1>
    </div>
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.printers.update', $printer) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('mac_address') is-invalid @enderror" id="mac_address" name="mac_address" value="{{ old('mac_address', $printer->mac_address) }}" required>
                    @error('mac_address')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <div class="form-check form-switch p-0 ps-5 fs-5">
                        <input class="form-check-input ms-n5" type="checkbox" role="switch" id="is_online" name="is_online" value="1" {{ old('is_online', $printer->is_online) ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary px-4">@lang('printer.update_submit')</button>
                </div>
            </form>
        </div>
    </div>
